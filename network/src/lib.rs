...
impl NetworkService {
    pub fn new(
        ...
        // Try bootstraps and any previously observed peer addresses immediately.
    ...
    async fn handle_swarm_event<E: std::fmt::Debug>(
        ...
        match event {
            ...
            SwarmEvent::Behaviour(behaviour::NodeBehaviourEvent::Gossipsub(
                ...
                self.handle_gossipsub_message(message, propagation_source)
                    .await;
            }
        ...
        match topic {
            TRANSACTION_TOPIC => {
                ...
                // Check rate limit
                {
                    ...
                    if let Err(e) = limiter.check_and_consume(&source, MessageType::Transaction) {
                        warn!(
                            "Rate limit exceeded for transaction from {:?}: {}",
                            source, e
                        );
            ...
            CONSENSUS_TOPIC => {
                // Check rate limit
                {
                    ...
                    {
                        warn!(
                            "Rate limit exceeded for consensus message from {:?}: {}",
                            source, e
                        );
                        return;
                    }
    ...
    fn try_dial_known_addr(&mut self, addr: Multiaddr) {
        debug!("Dialing known peer address: {}", addr);
        if let Err(e) = self.swarm.dial(addr.clone()) {
            debug!("Dial attempt skipped for {}: {}", addr, e);
        }
    }
}
