...
pub trait Executor {
    ...
    fn execute_block(&self, block: &Block) -> Result<(Hash, Vec<TransactionReceipt>), Box<dyn Error>>;
    
    /// Execute a single transaction
    fn execute_transaction(&self, tx: &Transaction, state_root: &Hash) -> Result<TransactionReceipt, Box<dyn Error>>;
...
pub trait State {
    ...
    /// Set storage value
    fn set_storage(&mut self, address: &Address, key: &[u8; 32], value: &[u8; 32]) -> Result<(), Box<dyn Error>>;
