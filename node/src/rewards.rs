...
impl RewardCalculator {
    ...
    pub fn calculate_block_rewards(
        ...
        *self.total_blocks_produced.entry(block.header.validator_set_id.to_le_bytes().to_vec()).or_insert(0) += 1;
    ...
    pub fn get_validator_stats(&self) -> Vec<(Vec<u8>, u64, u64)> {
        ...
        let all_keys: std::collections::HashSet<_> = self.total_blocks_produced
            .keys()
...
impl RewardManager {
    ...
    pub fn process_block(
        ...
        let rewards = self.calculator.calculate_block_rewards(
            block, proposer, voters, total_fees
        );
    ...
    pub fn has_penalties(&self, validator: &[u8]) -> bool {
        self.pending_rewards.get(validator).map_or(false, |&v| v < 0)
    }
}
