...
impl Validator {
    ...
    fn validate_governance(governance: &crate::config::GovernanceConfig) -> Result<()> {
        let proposal_deposit: u128 = governance.proposal_deposit.parse()
            .map_err(|_| anyhow!("Invalid proposal_deposit format: {}", governance.proposal_deposit))?;
        ...
        let quorum: f64 = governance.quorum_threshold.parse()
            .map_err(|_| anyhow!("Invalid quorum threshold format: {}", governance.quorum_threshold))?;
        
        if !(0.0..=1.0).contains(&quorum) {
            return Err(anyhow!("Quorum threshold must be between 0.0 and 1.0"));
        }
        
        let approval: f64 = governance.approval_threshold.parse()
            .map_err(|_| anyhow!("Invalid approval threshold format: {}", governance.approval_threshold))?;
        ...
        if approval <= quorum {
            return Err(anyhow!("Approval threshold ({}) must be greater than quorum threshold ({})", approval, quorum));
        }
    ...
    fn validate_economic(economic: &crate::config::EconomicConfig) -> Result<()> {
        let initial_reward: u128 = economic.initial_block_reward.parse()
            .map_err(|_| anyhow!("Invalid initial_block_reward format: {}", economic.initial_block_reward))?;
    ...
    fn validate_validators(validators: &[crate::config::ValidatorConfig], min_stake_str: &str) -> Result<()> {
        ...
        for validator in validators {
            ...
            if !public_keys.insert(&validator.public_key) {
                return Err(anyhow!("Duplicate validator public key: {}", validator.public_key));
            }
            
            // Validate address format
            let addr_clean = validator.address.trim_start_matches("0x");
            if addr_clean.len() != 40 {
                return Err(anyhow!("Invalid address format for {}: must be 20 bytes (40 hex chars)", validator.address));
            }
            ...
            let stake: u128 = validator.stake.parse()
                .map_err(|_| anyhow!("Invalid stake format for {}: {}", validator.address, validator.stake))?;
            ...
            let commission: f64 = validator.commission_rate.parse()
                .map_err(|_| anyhow!("Invalid commission rate for {}: {}", validator.address, validator.commission_rate))?;
            ...
            if pk_clean.len() != 64 && pk_clean.len() != 66 {
                return Err(anyhow!("Invalid public key format for {}: must be 32 or 33 bytes", validator.address));
            }
    ...
    fn validate_accounts(accounts: &[crate::config::AccountConfig]) -> Result<()> {
        ...
        for account in accounts {
            ...
            if addr_clean.len() != 40 {
                return Err(anyhow!("Invalid address format for {}: must be 20 bytes (40 hex chars)", account.address));
            }
            ...
            let balance: u128 = account.balance.parse()
                .map_err(|_| anyhow!("Invalid balance format for {}: {}", account.address, account.balance))?;
    ...
    fn validate_precompiles(precompiles: &[crate::config::PrecompileConfig]) -> Result<()> {
        ...
        for precompile in precompiles {
            ...
            if addr_clean.len() != 40 {
                return Err(anyhow!("Invalid precompile address: {}", precompile.address));
            }
            
            // Validate contract type
            if !valid_types.contains(&precompile.contract_type.as_str()) {
                return Err(anyhow!("Invalid precompile type: {}. Valid: {:?}", 
                    precompile.contract_type, valid_types));
            }
        }
    ...
    fn validate_cross_references(config: &GenesisConfig) -> Result<()> {
        ...
        let validator_addresses: HashSet<_> = config.validators.iter()
            .map(|v| &v.address)
            .collect();
        ...
        if treasury != "0000000000000000000000000000000000000000" {
            ...
            if !config.accounts.iter().any(|a| a.address == treasury_addr) {
                return Err(anyhow!("Treasury address {} not found in accounts", treasury_addr));
            }
        }
    ...
    pub fn print_summary(config: &GenesisConfig) -> Result<()> {
        ...
        println!("Initial reward:     {}", config.economic.initial_block_reward);
        println!("Halving interval:   {}", config.economic.halving_interval);
        println!("Fee burn:           {}%", config.economic.fee_burn_percentage);
