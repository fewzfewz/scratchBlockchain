...
pub fn parse_address(addr_str: &str) -> Result<[u8; 20]> {
    ...
    let bytes = hex::decode(addr_str).map_err(|e| anyhow!("Invalid address hex: {}", e))?;
...
impl GenesisBuilder {
    ...
    pub fn build(config: GenesisConfig) -> Result<GenesisOutput> {
        ...
        let validators = config
            ...
            .map(|v| {
                Ok(ValidatorOutput {
                    ...
                    commission_rate: v.commission_rate.parse().unwrap_or(0.05),
        ...
        let initial_block_reward: u128 =
            config.economic.initial_block_reward.parse().map_err(|_| {
                anyhow!(
                    "Invalid initial_block_reward: {}",
        ...
        Ok(GenesisOutput {
            ...
            governance_params: GovernanceParams {
                proposal_deposit: config.governance.proposal_deposit.parse().map_err(|_| {
                    anyhow!(
                        "Invalid proposal_deposit: {}",
                        config.governance.proposal_deposit
                    )
                })?,
                ...
                approval_threshold: config.governance.approval_threshold.parse().unwrap_or(0.5),
