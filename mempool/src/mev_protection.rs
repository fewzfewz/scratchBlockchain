...
impl MevProtection {
    ...
    pub fn commit_transaction(
        ...
        *counter += 1;
        ...
        let commitment =
            TransactionCommitment::new(tx_hash, current_height, current_round, commit_index);
    ...
    pub fn reveal_transaction(&self, tx: Transaction, current_height: u64) -> Result<()> {
        let tx_hash = tx.hash();
        ...
        if tx_hash != commitment.commitment {
            return Err(anyhow::anyhow!(
                "Transaction hash does not match commitment"
            ));
        }
    ...
    pub fn get_ready_transactions(&self, limit: usize) -> Vec<Transaction> {
        ...
        let remaining: VecDeque<RevealedTransaction> =
            all_revealed.into_iter().skip(limit).collect();
    ...
    pub fn remove_transactions(&self, txs: &[Transaction]) {
        ...
        if removed_count > 0 {
            debug!(
                "Removed {} revealed transactions after block inclusion",
                removed_count
            );
        }
    }
    ...
    pub fn cleanup_expired(&self, current_height: u64) {
        ...
        commitments.retain(|hash, commitment| {
            ...
            if is_expired {
                debug!(
                    "Expired commitment: {:?} from height {}",
                    hex::encode(&hash[..4]),
        ...
        if expired_count > 0 {
            info!(
                "Cleaned up {} expired commitments at height {}",
