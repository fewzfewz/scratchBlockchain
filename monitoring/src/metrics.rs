...
impl BlockchainMetrics {
    pub fn new() -> Result<Self, prometheus::Error> {
        ...
        // Block metrics
        let block_height =
            IntGauge::with_opts(Opts::new("chain_block_height", "Current block height"))?;
        ...
        // Transaction metrics
        let tx_total = IntCounter::with_opts(Opts::new(
            "chain_transactions_total",
            "Total transactions processed",
        ))?;

        let tx_pending = IntGauge::with_opts(Opts::new(
            "chain_transactions_pending",
        ...
        // Validator metrics
        let validator_count = IntGauge::with_opts(Opts::new(
            "chain_validator_count",
            "Number of active validators",
        ))?;

        let validator_stake_total = Gauge::with_opts(Opts::new(
            "chain_validator_stake_total",
            "Total staked amount",
        ))?;

        let missed_blocks = IntCounter::with_opts(Opts::new(
            "chain_missed_blocks_total",
            "Total missed blocks",
        ))?;

        // Network metrics
        let peer_count =
            IntGauge::with_opts(Opts::new("chain_peer_count", "Number of connected peers"))?;

        let network_bytes_sent = IntCounter::with_opts(Opts::new(
            "chain_network_bytes_sent_total",
            "Total bytes sent",
        ))?;

        let network_bytes_received = IntCounter::with_opts(Opts::new(
            "chain_network_bytes_received_total",
            "Total bytes received",
        ))?;

        // Consensus metrics
        let consensus_rounds = IntCounter::with_opts(Opts::new(
            "chain_consensus_rounds_total",
        ...
        // Storage metrics
        let state_size_bytes = IntGauge::with_opts(Opts::new(
            "chain_state_size_bytes",
