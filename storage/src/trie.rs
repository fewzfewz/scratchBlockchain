...
pub enum TrieNode {
    ...
    Extension { path: NibblePath, child: NodeHash },
...
impl PatriciaTrie {
    ...
    pub fn new(db: Arc<dyn KeyValueStore>) -> Result<Self, Box<dyn Error>> {
        ...
        let (root, cache) = if let Some(root_bytes) = db.get(ColumnFamily::State, b"root")? {
        ...
        let new_node = match node {
            ...
            TrieNode::Extension {
                ...
                if common_len == ext_path.len() {
                    ...
                    let (new_child, child_changes) =
                        self.insert_at(child, &path[common_len..], value)?;
    ...
    fn delete_at(
        ...
        let new_node = match node {
            ...
            TrieNode::Extension {
                ...
                if path.starts_with(ext_path) {
                    let (new_child, child_changes) =
                        self.delete_at(child, &path[ext_path.len()..])?;
                    changed.extend(child_changes);
